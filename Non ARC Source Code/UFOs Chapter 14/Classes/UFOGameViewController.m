...
-(void)viewDidLoad
{
	...
	for(int x = 0; x < 5; x++)
	{
        [self performSelector:@selector(spawnCow) withObject:nil afterDelay:0.5];
    }
...
-(void) movePlayer:(float)vertical :(float)horizontal;
{
	...
	if ((vertical < 0 && playerFrame.origin.y < self.view.bounds.size.height) || (vertical > 0 && playerFrame.origin.y > 20))
		playerFrame.origin.y -= vertical*movementSpeed;
	
	if ((horizontal < 0 && playerFrame.origin.x < self.view.bounds.size.width) || (horizontal > 0 && playerFrame.origin.x > 0))
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	if(currentAbductee)
	{
		...
		frame.origin.y = height-(height/10);
...
-(void)spawnCow;
{
	...
	UIImageView *cowImageView = [[UIImageView alloc] initWithFrame:CGRectMake(arc4random()% width, height-(height/10), 64, 42)];
...
-(void)updateCowPaths
{
	...
	for(int x = 0; x < [cowArray count]; x++)
	{
		...
		if(tempCow != currentAbductee)
		{
			...
			if(tempCow != currentAbductee)
				tempCow.frame = CGRectMake(newX, height-(height/10), 64, 42);
